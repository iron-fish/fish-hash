...
namespace FishHash
{
	...
	******************************/
	...
	fnv1(uint32_t u, uint32_t v) noexcept
	{
	...
	******************************/
	...
	void hash(uint8_t *output, const fishhash_context *ctx, const uint8_t *header, uint64_t header_size) noexcept
	{
		...
		if (!output)
			output = static_cast<uint8_t *>(std::calloc(1, 32));
	...
	******************************/
	...
	fishhash_context *get_context(bool full) noexcept
	{
		...
		if (shared_context)
		{
			...
			if ((!full) || (shared_context->full_dataset))
		...
		if (!alloc_data)
			return nullptr; // Signal out-of-memory by returning null
	...
	// void keccak2(uint64_t *out, size_t bits, const uint8_t *data, size_t size);
	void keccak2(uint64_t *out, size_t bits, const uint8_t *data, size_t size) noexcept
	{
		keccak(out, bits, data, size);
	}
}
